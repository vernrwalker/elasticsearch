...
api = Api(app, 
        version='2.0', 
        title='App Service ML API',
        description='API to compute severty predictions from sentences',
        )
ns = api.namespace('UNISYS_ServiceNow', description='ML Model of incident prediction')
...
def modelTrainer():
    ...
    df = pd.read_pickle("./incidents.pkl")
...
prediction_model = api.model("predictor", {
    "text": fields.String(description="the service_now text", required=True, default="null",example='Customer didnot receive eFax')
    })

@ns.route('/Severity/<string:text>')  
class Predict(Resource):
    @ns.doc('use this to predict a severity values')
    # @api.expect(prediction_model)
    def get(self,text):
        logging.warning('use this to predict a severity values')
        try:
            if ( app.gModel == None):
                modelTrainer()
