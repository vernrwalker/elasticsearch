...
# THIS IS A SET OF QUERIES FOR ELASTICSEARCH, USING KEYWORDS OF RESULTS, FORMULATED AND TESTED WITH KIBANA (run on localhost:5601)
    ...
    query = {
        ...
        "aggregations": {
            "attributiontypes": {
                "filters": {
                    "filters": {
                        "positive": {"match": {"attributions.polarity": "positive"}},
                        "negative": {"match": {"attributions.polarity": "negative"}},
                        "undecided": {"match": {"attributions.polarity": "undecided"}}
                    }
                }
            }
        }
